...
const PublicProfilePage = () => {
  const { userId } = useParams<{ userId: string }>(); // URL'den ID'yi al
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // 1. Kullanıcı bilgilerini çek (Yeni endpoint)
        const userRes = await api.get(`/users/${userId}`);
        setUser(userRes.data);

        // 2. Kullanıcının yazılarını çek (Mevcut endpoint)
      ...
      <div className="bg-gray-800 p-8 rounded-lg shadow-xl mb-10 flex flex-col md:flex-row items-center md:items-start gap-6 text-center md:text-left">
        ...
        <div className="shrink-0">
            {user.profileImage ? (
              ...
              <div className="w-32 h-32 rounded-full bg-gray-700 border-4 border-gray-600 flex items-center justify-center text-gray-500 text-4xl uppercase">
        ...
        <div className="flex-1">
            ...
            {user.bio && (
                <div className="bg-gray-900/50 p-4 rounded-lg text-gray-300 italic">
                    "{user.bio}"
                </div>
            )}
        </div>
